...
// --- Constructor ---
LibraryService::LibraryService(infra::IRepository &r) : repo(r) {
  books = repo.loadBooks();
  members = repo.loadMembers();
  loans = repo.loadLoans();
}
...
LibraryService::searchBooks(const std::string &query) const {
...
std::vector<domain::Book> LibraryService::getAllBooks() const { return books; }
...
domain::Member *LibraryService::getMember(int id) { return findMemberById(id); }
